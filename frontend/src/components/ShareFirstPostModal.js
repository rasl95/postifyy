...
import { Sparkles, Copy, Check, PartyPopper, Gift, Zap, X } from 'lucide-react';
...
// Mobile-first order: Telegram & WhatsApp first
const SHARE_PLATFORMS_MOBILE = [
  ...
  { id: 'twitter', name: 'X', icon: TwitterIcon, color: 'bg-white/10 hover:bg-white/20 text-white',
...
// Desktop order: X first
const SHARE_PLATFORMS_DESKTOP = [
  ...
  const PlatformButton = ({ platform }) => {
    ...
    return (
      <button
        ...
        className={cn(
          'relative flex flex-col items-center gap-1 sm:gap-1.5 p-2.5 sm:p-3 rounded-xl border transition-all active:scale-95 sm:hover:scale-105',
          isShared ? 'border-green-500/30 bg-green-500/10' : `border-white/5 ${platform.color}`
        )}
        data-testid={`share-${platform.id}-btn`}
      >
        {isShared && (
          <div className="absolute -top-1 -right-1 sm:-top-1.5 sm:-right-1.5 w-4 h-4 bg-green-500 rounded-full flex items-center justify-center">
            <Check className="w-2.5 h-2.5 text-white" />
          </div>
        )}
        <Icon className={cn('w-5 h-5 sm:w-5 sm:h-5', isShared && 'text-green-400')} />
        <span className={cn('text-[10px] font-medium leading-tight', isShared ? 'text-green-400' : 'opacity-80')}>{platform.name}</span>
      </button>
    );
  };

  return (
    <DialogPrimitive.Root open={isOpen} onOpenChange={handleDismiss}>
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content
          className={cn(
            'fixed z-50 bg-[#0A0A0B] border-white/10 border shadow-xl',
            'w-full max-h-[90dvh] overflow-y-auto overscroll-contain',
            // Mobile: bottom sheet
            'bottom-0 left-0 right-0 rounded-t-2xl',
            // Desktop: centered dialog
            'sm:bottom-auto sm:left-[50%] sm:top-[50%] sm:translate-x-[-50%] sm:translate-y-[-50%] sm:max-w-lg sm:rounded-2xl',
            // Animations
            'data-[state=open]:animate-in data-[state=closed]:animate-out',
            'data-[state=open]:slide-in-from-bottom data-[state=closed]:slide-out-to-bottom',
            'sm:data-[state=open]:slide-in-from-bottom-0 sm:data-[state=open]:slide-in-from-left-1/2 sm:data-[state=open]:fade-in-0 sm:data-[state=open]:zoom-in-95',
          ...
          data-testid="share-first-post-modal"
        >
          {/* Top gradient bar */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-[#FF3B30] via-[#FF6A3D] to-[#FFD60A] rounded-t-2xl" />
          ...
          <DialogPrimitive.Close className="absolute right-3 top-3 rounded-full p-1.5 opacity-60 hover:opacity-100 transition-opacity hidden sm:flex items-center justify-center text-gray-400 hover:text-white">
            <X className="h-4 w-4" />
            <span className="sr-only">Close</span>
          </DialogPrimitive.Close>

          <div className="px-5 pb-5 pt-2 sm:p-6 sm:pt-5">
            {/* Header */}
            <div className="text-center">
              <div className="relative">
                {rewardState === 'rewarded' ? (
                  <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 rounded-2xl bg-green-500/15 border border-green-500/30 flex items-center justify-center animate-in zoom-in duration-300">
                    <Check className="w-6 h-6 sm:w-8 sm:h-8 text-green-400" />
                  </div>
                ) : (
                  <div className="w-12 h-12 sm:w-16 sm:h-16 mx-auto mb-3 sm:mb-4 rounded-2xl bg-gradient-to-br from-[#FF3B30]/20 to-[#FFD60A]/10 border border-[#FF3B30]/30 flex items-center justify-center">
                    <Sparkles className="w-5 h-5 sm:w-7 sm:h-7 text-[#FF3B30]" />
                  </div>
                )}
                <div className="absolute -top-1 right-4 sm:right-24 w-6 h-6 sm:w-8 sm:h-8 text-[#FFD60A] animate-bounce">
                  <PartyPopper className="w-5 h-5 sm:w-6 sm:h-6" />
                </div>
              </div>

              <h2 className="text-lg sm:text-2xl font-bold text-white" data-testid="share-modal-title">
                {rewardState === 'rewarded'
              ...
              <p className="text-gray-400 mt-1 text-sm sm:text-base">
                {rewardState === 'rewarded'
                  ? (language === 'ru' ? 'Спасибо! Продолжайте создавать.' : 'Thanks for sharing! Keep creating.')
                  : (language === 'ru' ? 'Поделитесь и получите +3 генерации' : 'Share and get +3 free generations')}
              </p>
            </div>

            {/* Reward incentive banner */}
            {rewardState !== 'rewarded' && !bonusClaimed && (
              <div className="bg-gradient-to-r from-[#FF3B30]/10 to-[#FFD60A]/10 border border-[#FF3B30]/20 rounded-xl p-2.5 sm:p-3 flex items-center gap-2.5 sm:gap-3 mt-3 sm:mt-4" data-testid="share-reward-banner">
                <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg bg-[#FF3B30]/20 flex items-center justify-center flex-shrink-0">
                  <Gift className="w-4 h-4 sm:w-5 sm:h-5 text-[#FF3B30]" />
                </div>
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-gray-400 truncate">
                    {language === 'ru' ? 'Поделитесь на любой платформе' : 'Share on any platform to claim'}
                  </p>
                </div>
                <div className="ml-auto flex-shrink-0">
                  <div className="px-2 py-0.5 sm:px-2.5 sm:py-1 bg-[#FF3B30] text-white text-[10px] sm:text-xs font-bold rounded-full">FREE</div>
                </div>
              </div>
            )}
            ...
            {/* Reward success */}
            {rewardState === 'rewarded' && (
              <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-3 sm:p-4 text-center animate-in slide-in-from-bottom duration-300 mt-3 sm:mt-4" data-testid="share-reward-success">
                <div className="flex items-center justify-center gap-2 mb-0.5">
                  <Zap className="w-5 h-5 text-green-400" />
                  <span className="text-lg font-bold text-green-400">+{BONUS_CREDITS}</span>
                </div>
                <p className="text-xs text-green-400/70">
                  {language === 'ru' ? 'Бонусные генерации добавлены' : 'Bonus generations added to your account'}
                </p>
              </div>
            )}

            {/* Content preview — DESKTOP ONLY */}
            {contentPreview && rewardState !== 'rewarded' && (
              <div className="hidden sm:block bg-white/[0.03] border border-white/5 rounded-xl p-3.5 mt-3" data-testid="share-content-preview">
            ...
            <div className="mt-4 sm:mt-4">
              <p className="text-[10px] sm:text-xs text-gray-500 mb-2.5 sm:mb-3 text-center font-medium uppercase tracking-wider">
                {rewardState === 'rewarded'
                  ? (language === 'ru' ? 'Поделиться ещё' : 'Share more')
                  : (language === 'ru' ? 'Выберите платформу' : 'Choose a platform')}
              </p>
              
              {/* Mobile: 2x2 grid, Telegram/WhatsApp top row */}
              <div className="grid grid-cols-4 gap-2 sm:hidden">
                {SHARE_PLATFORMS_MOBILE.map(p => <PlatformButton key={p.id} platform={p} />)}
              </div>
              
              {/* Desktop: 4-col row */}
              <div className="hidden sm:grid grid-cols-4 gap-2">
                {SHARE_PLATFORMS_DESKTOP.map(p => <PlatformButton key={p.id} platform={p} />)}
              </div>
            </div>

            {/* Referral link copy */}
            <div className="mt-3 sm:mt-4">
              <p className="text-[10px] sm:text-xs text-gray-500 mb-1.5 sm:mb-2 text-center">
                {language === 'ru' ? 'Ваша реферальная ссылка' : 'Your referral link'}
              </p>
              <div className="flex items-center gap-2">
                <div className="flex-1 bg-white/5 border border-white/10 rounded-lg px-3 py-2 sm:py-2.5 text-[11px] sm:text-xs text-gray-400 truncate font-mono" data-testid="share-referral-link">
                  {referralUrl}
                </div>
                <Button
                  size="sm" variant="outline"
                  onClick={copyReferralLink}
                  className="h-8 sm:h-9 px-2.5 sm:px-3 border-white/10 text-white hover:bg-white/10 shrink-0"
            ...
            {/* Bonus callout */}
            <div className="bg-[#FF3B30]/5 border border-[#FF3B30]/15 rounded-lg p-2 sm:p-2.5 mt-2.5 sm:mt-3 text-center">
              <p className="text-[11px] sm:text-xs text-[#FF3B30] font-medium">
                {language === 'ru'
                  ? '+3 генерации за каждого друга по ссылке'
                  : '+3 generations for every friend who signs up'}
              </p>
            </div>
